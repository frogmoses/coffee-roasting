...
| Command | What it does |
|---------|-------------|
| `cupping <id> -i` | **Guided tasting questions** (sour/roasty, astringency, sweetness, body, better/worse than last batch) — no flavor vocabulary needed; regenerates recommendations |
| `cupping <id> -n "notes"` | Attach free-text tasting notes to a roast **and regenerate its recommendations** with the flavor signal |
| `cupping <id>` | View existing cupping notes |
| `plan [id] --dev 150,90,210` | Print a **contrast set** for the next session: same recipe through first crack, three different FC→DROP times, projected drop temps, heat-soak and safety notes |
| `bean <name>` | Look up a bean in find-coffee |

Adding cupping notes is the key feedback step: the scan-time recommendations were
generated before you tasted the roast, so `cupping -n` re-runs the analysis with
how it actually cupped (if the API is unreachable, your notes still save and the
old recommendations are kept). Re-scanning with `--force` preserves any cupping
notes you've added and also feeds them into the analysis.

### Tasting without a trained palate

You do not need to name flavors to steer a roast. `cupping <id> --intake` asks a
handful of questions anyone can answer — sour/bright vs bitter/roasty (the
development axis), astringency, sweetness, body, and whether this batch is
better or worse than the last one. The answers are stored structured and also
rendered into the notes the recommender reads, with a legend telling it how each
axis maps to roast mechanics. Preference beats description: "better than last
batch" is the signal the dial-in runs on.

Roast for contrast, not repetition: `plan` takes your latest recipe and lays out
three batches that differ only in seconds after first crack (control, short,
long). Tasting the bracket is how the middle becomes describable.
...
**Recommendations** are generated by Claude reading your full log — the move-by-move heater/fan timeline, a downsampled BT/RoR curve, any physical observations you flagged in Artisan (tipping, scorching, quiet first crack…), and up to three previous roasts of the same bean (what was advised after each and how it cupped) — so the advice points at specific dial moves and moments ("ease the heater to ~80% by 250F", "make one cut around 340F and hold it through first crack") and builds on your dial-in history rather than repeating generic targets. They're prioritized:
- `[!!!]` — fix this first
- `[ ! ]` — worth improving
- `[   ]` — info
...
1. **Flavor** — what the bean is *supposed* to taste like (from the bean profile) versus how it *actually* cupped (your tasting notes). Add your notes with `cupping <id> -n "..."` — that command immediately re-runs the analysis with them, and later roasts of the same bean see them as history.
